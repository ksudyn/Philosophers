...
/*   Updated: 2025/12/09 13:08:43 by ksudyn           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	philo_info(t_philosophers *philo, int id, t_rutine *rutine)
{
	...
	philo->count_eaten = 0; // el sexto argumento, es 0 si no hay o si aun no han llegado a ahacer tantas veces la rutina
	// printf("%i, philo->id_philo \n", philo->id_philo);
	// printf("%lli, philo->act_time \n", philo->act_time);
	// printf("%i, philo->left_fork \n", philo->left_fork);
	// printf("%i philo->right_fork \n", philo->right_fork);
	// printf("%i, philo->count_eaten \n", philo->count_eaten);
}
...
int	init_philosophers(t_rutine *rutine)
{
	...
	rutine->philosophers_full = 0;
		// cantidad de filósofos que ya comieron lo suficiente, se inicia a 0
	rutine->status = 1;           
		// se mantien en 1 si ningun filosofo muere al morir se pone en 0
	rutine->dead = 0;
